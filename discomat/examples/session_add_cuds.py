...
for g in lg:
    query = query_lib.subject_graph(subj)
    query = query_lib.augment_graph_query(query, g)
    print(query)
...
query = query_lib.all_triples(subj, None, None)  # should give the same result as above.
print(query)
query = query_lib.all_triples(None, RDF.type, CUDS.Cuds)  # should give the same result as above.
print(query)
